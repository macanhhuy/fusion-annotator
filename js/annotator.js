...
// id returns an identifier unique within this session
        ...
        HttpStorage.prototype.query = function (queryObj) {
            return this._apiRequest('search', queryObj)
                .then(function (obj) {
                    var rows = obj.rows;
                    window.totalAnnotations = obj.total;
                    window.annotations = rows;
                    var event = new Event('totalAnnotations');
        ...
        var Editor = exports.Editor = Widget.extend({
            ...
            addField: function (options) {
                ...
                if (field.type === 'textarea') {
                    ...
                    input = $('<h6 class="topic-title">' + field.label + '</h6>');
                }
                ...
                if (field.type === 'checkbox') {
                    input.attr({
                        ...
                        'data-question': field.questionValue,
                    });
                ...
                if (field.isQuestion) {
                    this.element.find('.annotator-questions ').append(element);
                } else {
...
// HTML template for this.element.
        Editor.template = [
            '<div class="annotator-outer annotator-editor annotator-hide col-md-12">',
            '  <form class="annotator-widgets row">',
            '  <div class="annotator-widget">',
            '    <ul class="annotator-actions col-md-4 "></ul>',
            '    <div class="annotator-questions col-md-8 "><h5 class="annotator-heading">Select one or more appropriate Questions</h5></div>',
            '    <div class="annotator-controls col-md-12">',
...
// Returns a new instance of the Filter.
        var Filter = exports.Filter = function Filter(options) {
            ...
            document.addEventListener('nextAnnotator', function (e) {
                self._onNextClick(e);
            });

            document.addEventListener('prevAnnotator', function (e) {
                self._onPreviousClick(e);
            });
...
// Returns nothing.
        Filter.prototype._scrollToHighlight = function (highlight) {
            ...
            //huymac 02-03-2016
             window.currentAnnotationId = $('.annotator-hl-active').data('annotation-id');
             var event = new Event('annotationId');
             document.dispatchEvent(event);
        };
        ...
        exports.viewerExtension = function viewerExtension(v) {
            function updateViewer(field, annotation) {
                ...
                if (annotation.tags &&
                    ...
                    field.prepend('<strong>Tags: </strong><br>');
                } else {
                    field.remove();
                }
            }
 ...
 *     })
        ...
        exports.editorExtension = function editorExtension(e) {
            ...
            function setAnnotationTags(field, annotation) {
                annotation.tags = input.val().split(',');
            }
...
//                        self.load(annotations, util.mousePosition(event));
                        var position = util.mousePosition(event);
                        window.currentAnnotation = annotations[0];
                        $('.annotator-viewer').html('<button class="md-icon-button md-ink-ripple md-button" type="button" ng-transclude="" onclick="editAnnotation(' + window.currentAnnotation.id + ')">'  +
                        '<md-icon class="ng-scope ng-isolate-scope material-icons">mode_edit</md-icon>'+
